"use strict";
var fs = require('fs'), 
mvc_loader_view_model = require('./View.js').view;
...
var that = {
    ...
    settings: {
        ...
        routesFilePath: __dirname,
        viewModel: mvc_loader_view_model
    },
    attr: {
        /* Views container */
        views: {},
    ...
    **/
    load: function(settings, callback) {
        for(var i in settings) {
            if (this.settings[i] != undefined) {
                this.log(1, 'mvc.settings.' + i + ' is overwritten by ' + settings[i].toString() + '.');
            }
    ...
    loadViews: function(callback) {
        this.exploreFolderSync(this.settings.viewsPath);
        callback();
    },
    exploreFolderSync: function(path) {
        var pathname=null,
        stat=null,
        index=null,
        ...
        viewPath=null,
        slash='/';
        if (path.charAt(path.length-1) != '/') { path = path.concat(slash) }
        var folderContent = fs.readdirSync(path); // get folder content
        for(var n=0;n<folderContent.length;n++) {
            if (/^.*~$/.exec(folderContent[n])) { continue; }
            pathname = path.concat(folderContent[n]);
            stat = fs.statSync(pathname); // get file stat
            if (stat.isFile()) {
                index = /(\.[^.]+)$/.exec(pathname).index;
                ...
                if (this.settings.ENV == 'dev') { // dev modes
                    if(that.attr.views[viewPath] != null) { 
                        this.log(2, 'the path ' + pathFileWithoutExt + ' already exists. ' + that.attr.views[viewPath]);
                    }
                    this.log(this.DEBUGMODE['verbose'], '\u001b[32mLOAD VIEW PATH\u001b[0m ' + pathname + ' \u001b[32minto\u001b[0m ' + viewPath);
                    that.attr.views[viewPath] = pathname;
                } else if (this.settings.ENV == 'prod') { // prod mode
                    this.log(this.DEBUGMODE['verbose'], '\u001b[32mLOAD VIEW\u001b[0m: ' + pathFileWithoutExt);
                    if(that.attr.views[viewPath] != null) { 
                        this.log(this.DEBUGMODE['warning'], 'the path ' + pathFileWithoutExt + ' already exists. ' + that.attr.views[viewPath]);
                    }
                    that.attr.views[viewPath] = new this.settings.viewModel(pathname);
                }
            } else {
                this.exploreFolderSync(pathname);
            }
        }
    },
    setRoute: function(controller, method, action, url) {
        ...
        if (!controller || typeof(controller) != 'string') {
            this.log(this.DEBUGMODE['error'], 'The controller is not defined or this is not a string');
        }
        ...
        if (this.settings.ENV == 'dev') { // dev modes
            this.log(this.DEBUGMODE['verbose'], '\u001b[32mSET ROUTE\u001b[0m: ' + '[' + action.toUpperCase() + '] ' + url);
            ...
            this.attr.app[expressAction](url, (function(path, controllerName, method, loader) {
                return function(req, res, next) {
                    this.log(this.DEBUGMODE['verbose'], '\u001b[32mLOAD CONTROLLER\u001b[0m: ' + path);
                    // REQUIRE CONTROLLER
                    var controller = require(path)[controllerName];
                    controller.view = function(str) {
                        return new View(loader.attr.views[str]);
                    };
                    if (controller[method]) {
                        // CALL FUNCTION
                        controller[method](req, res, next);
                    } else {
                        this.log(this.DEBUGMODE['error'], 'The method \u001b[32m' + controllerName + '\u001b[0m.\u001b[32m' + method + '\u001b[0m required for \u001b[32m[' + action.toUpperCase() + '] ' + url + '\u001b[0m is not found !');
                    }
                }
            ...
            var that = this;
            this.log(this.DEBUGMODE['verbose'], '\u001b[32mLOAD CONTROLLER\u001b[0m: ' + controllerName + '.' + method);
            // REQUIRE CONTROLLER
    ...
    loadRoutesFromFile: function() {
        ...
        this.log(this.DEBUGMODE['verbose'], 'Load route file: ' + routesPath);
        if (!fs.existsSync(routesPath)) {
            this.log(this.DEBUGMODE['verbose'], 'File not found, create it !');
            fs.writeFileSync(routesPath, new Buffer('{}'));
        }
    ...
    errorMessage: function(err) {
        ...
        console.log(err);
        this.log(this.DEBUGMODE['error'], message);
    }
};
